...
SYNOPSIS:
    ...
    function my_ajax_callback ( result ){
        ...
        // data retrieved from Recruit Web Service,
        // either a javascript object (json) or a XMLDocument (xml).

        var entries_per_page = 10;
        var page = Recruit.UI.Page( result, entries_per_page );
...
DESCRIPTION:
    ...
    using with Recruit Web Service.
...
Recruit.UI.Page.prototype = new Data.Page();

Recruit.UI.Page.prototype.initialize =
function ( data, count ){
    var start = 1;
    var total = 0;
    if( !count ){ count = 10 };
...
Recruit.UI.Page.prototype.page_param =
function ( page, hash ){
    page = parseInt( page );
...
Recruit.UI.Page.prototype.next_page_param =
function ( hash ){
    return this.page_param( this.next_page(), hash );
}

Recruit.UI.Page.prototype.previous_page_param =
function ( hash ){
    return this.page_param( this.previous_page(), hash );
}

//
// Recruit.UI.Page.Simple
//
Recruit.UI.Page.Simple =
function ( data, count ){
    if( typeof( data ) != 'object' ){ return undefined }
    this.bid = 'rui';     // bid = 媒体ID
    this.cnm = 'Recruit'; // cnm = ベースのクラス名
    this.initialize.apply( this, arguments );
}

Recruit.UI.Page.Simple.prototype = new Recruit.UI.Page();

Recruit.UI.Page.Simple.prototype.paginate =
function ( hash ){
    var _self = this;
...
Recruit.UI.Page.Simple.prototype._get_template =
function ( id, prm ){
    var b = this.bid; 
    if( id == 'no_space' ){
        var tmpl = 
