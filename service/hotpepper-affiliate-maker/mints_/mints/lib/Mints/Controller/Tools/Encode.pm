...
sub _new_instance {
	...
	return bless({
		localeModule => $ctrl->getLocaleSetting(),
	},$class);
}
...
sub fromTo {
	...
	$self->{localeModule}->fromTo_preHook($_[0],$from,$to);
	return Encode::from_to($_[0],$from,$to);
}
...
sub guessAndFromTo {
	...
	if($enc ne Encode::resolve_alias($defaultEncoding)){
		$self->fromTo($_[0],$enc,$defaultEncoding);
	}
	return 1;
}
