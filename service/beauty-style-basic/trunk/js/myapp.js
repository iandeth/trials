...
MyApp.prototype = {
        ...
        this.tabs = $( '#tab-ul' ).tabs({
            ...
            click: function ( clicked, show, hide ){
                ...
                // 3つめのタブが押されたら
                    ...
                    $.historyLoad( _self.tabs.previous_hash.desc );
                }else{
                    $.historyLoad( 'top' );
                }
            }
        });
        this.tabs.actuall_user_click = true;  // jquery.history 組み合わせ対策
        this.tabs.previous_hash = {  // jquery.history 組み合わせ対策 
            list: '',
            desc: ''
        };
    ...
    ajax_get_list_onresponse : function ( tree ){
        ...
        if( res.error ){
            this.elm.list_page.top.html( '<span class="rui-page-error">'
    ...
    _make_results_list : function ( tree ){
        ...
        $( res.style ).each( function( i, r ){
            ...
            .click( function(){
                ...
                $.historyLoad( obj.hash, function (){
                    _self.tabs.actuall_user_click = false;
                    _self.tabs.previous_hash.desc = obj.hash;
    ...
    _create_desc_view: function ( id ){
        ...
        $( '<p><a href="' + r.salon.urls.pc + '" target=_"blank">' 
            + r.salon.name + '　(' + r.salon.name_kana + ')</a></p>' )
            .appendTo( tgt.info );
