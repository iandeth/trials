...
sub new {
	...
	# change standard output
        # won't work under mod_perl2.0.3 //2007-07
	# select($o{out});
...
sub resolveRunMode {
	...
	if(!-f $path){
		$self->setError("no such file (module): ${runMode}.pm");
		return undef;
	}
	# fix run mode
...
sub validateParam {
	...
	if($rm eq ''){
        ...
        # reset view module on error
        $self->setViewModule( $opt->{viewModuleOnError} ) if
            $opt->{viewModuleOnError};
