...
sub getLocaleSetting {
	my $self = shift;
	$self->loadModule( $self->{adm}{localeModule} );
	return $self->{adm}{localeModule};
}
...
sub setCookie {
	...
	foreach my $k (qw(name value expires)){
		$param{"-$k"} = $param{$k};
		delete $param{$k};
	}
	if($self->needTrEncoding){
...
sub redirect {
	...
	# define full url
	if($tgt !~ m|^http://|){
		$url = "http://$self->{env}{SERVER_NAME}$self->{env}{SCRIPT_NAME}/";
		$url .= ($self->{mints}{hideAppNameFromUrl})?
...
sub validate {
	...
	$CGI::Ex::Validate::ERROR_PACKAGE = 'Mints::Controller::CGI::ValidateError';
